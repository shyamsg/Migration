...
def comp_pw_coal_disc(m, Ne, t):
    ...
    """
    numpops = len(Ne)
    # also the boundary condition, at 0 generations
    coalInt = np.zeros((numpops, numpops))
    #stores last gen values for the intensities
...
def comp_pw_coal_cont(m, Ne_inv):
    ...
    for dp1 in demePairs.keys():
        ...
        for dp2 in demePairs.keys():
            if (dp1 == dp2):
                continue
            if (dp1[0] == dp1[1]): #both lines in same deme
                if (dp2[0] == dp2[1]):
...
def expM(Q):
    ...
    for i in xrange(len(L)):
        if abs(L[i]/slm) < EIG_THRES: L[i] = 0
    L = np.diag(np.exp(L))
    U = np.matrix(U)
    eQ = U*L*U.I
#             print (sl[0]/sl[-2])
...
#    ms = [temp*pc for temp in x[k:]]
    ...
    try:
#        tempo = (linalg.norm(obs_coal_rates-Pcurr, 2))**2
#        tempo = (linalg.norm((np.log(obs_coal_rates + 1e-200)/np.log(10)-np.log(Pcurr + 1e-200)/np.log(10)), 2))**2
...
#        tempo = bhatt_bern(obs_coal_rates, Pcurr) 
    except:
        ...
        for j in range(i, nd):
            ...
            for a in popD[i]:
                for b in popD[j]:
                    ...
                    if tt not in cols:  cols.append(tt)
            ptc.append(cols)
    return ptc
...
def compute_pw_coal_rates(Nes, ms, ts, popmaps):
 ...
 #       print 'P0:', oldnumpops, P0
        ...
        P = P0*eQ
#        print 'Q:', Q
#        print 'expM(tQ): ', np.real(eQ)
#        print 'P:', np.real(P)
        exp_rates[:,i] = np.real(P[0:nr-1,-1])
        P0 = P0*conv_scrambling_matrix(eQ)
#        print 'scramb:', np.real(conv_scrambling_matrix(eQ))
#        print 'new P0:', np.real(P0)
...
def comp_N_m(obs_rates, t, merge_threshold, useMigration):
...
#    m_threshold = m_threshold
    ...
    P0 = np.matrix(np.eye(nr))
#    N0 = np.random.uniform(1000,5000,numdemes)#+np.zeros(numdemes)
    N0_inv = np.random.uniform(5e-5, 1e-3, numdemes)
    lims = [(1e-15, 1e-1)]*numdemes
    m0 = np.random.uniform(1e-8, 1e-3, (numdemes*(numdemes-1))/2)#+np.zeros((numdemes*(numdemes-1))/2)
    lims += [(0,1e-1)]*((numdemes*(numdemes-1))/2)
    ...
    for i in xrange(numslices):
        ...
        if i > 0:
            x0 = bestxopt.copy()
            xopt, fun, nit, nfev, status = bnm.fmin_bound(compute_Frob_norm_mig, x0, bounds=lims, args=(t[i], obs_rates[:,i], P0, make_merged_pd(pdlist)), maxfun=100000, maxiter=10000, xtol=XTOL, ftol=FTOL, full_output=True, disp=False)
            bestxopt = xopt
            bestfval = fun
#            print 'prev', bestfval, [1./x for x in xopt[0:numdemes]], xopt[numdemes:]
#            for lll in xrange((numdemes*(numdemes-1))/2):
#                x0 = bestxopt.copy()
#                x0[numdemes+lll] = 9.9e-3
#                xopt, fun, nit, nfev, status = bnm.fmin_bound(compute_Frob_norm_mig, x0, bounds=lims, args=(t[i], obs_rates[:,i], P0, make_merged_pd(pdlist)), maxfun=100000, maxiter=10000, xtol=XTOL, ftol=FTOL, full_output=True, disp=False)
#                if (fun < bestfval):
#                    bestfval = fun
#                    bestxopt = xopt
#                print 'high mig', lll, fun, [1./x for x in xopt[0:numdemes]], xopt[numdemes:]
#        xopt() to get the N, m for this slice
#        Use those N, m as starting points for next slice
#        update the P0 matrix - have to update this to mean different things
        else:
            bestxopt = None
            bestfval = 1e200
        reestimate = True
...
#            N0 = np.random.uniform(1000,5000,numdemes)#+np.zeros(numdemes)
            ...
            N0_inv = np.random.uniform(5e-5,1e-3, numdemes)
            m0 = np.random.uniform(8e-3, 1e-3, (numdemes*(numdemes-1))/2)
            x0 = np.hstack((N0_inv, m0))
            lims = [(1e-15, 1e-1)]*numdemes
            lims += [(0,1e-1)]*((numdemes*(numdemes-1))/2)
            xopt, fun, nit, nfev, status = bnm.fmin_bound(compute_Frob_norm_mig, x0, bounds=lims, args=(t[i], obs_rates[:,i], P0, pdslice), maxfun=100000, maxiter=10000, xtol=XTOL, ftol=FTOL, full_output=True, disp=False)
            if (fun < bestfval):
                bestfval = fun
                bestxopt = xopt
#            print 'high', fun, [1./x for x in xopt[0:numdemes]], xopt[numdemes:]
            N0_inv = np.random.uniform(5e-5, 1e-3, numdemes)
            m0 = np.random.uniform(1e-8, 1e-3, (numdemes*(numdemes-1))/2)#+np.zeros((numdemes*(numdemes-1))/2)
            x0 = np.hstack((N0_inv,m0))
            nrestarts = 0
            ...
            if (fun < bestfval):
                ...
                #print 'resetting ',
            print nrestarts
#            print nrestarts, fun, [1./x for x in xopt[0:numdemes]], xopt[numdemes:]
            while nrestarts < RESTARTS and bestfval > FLIMIT :
                #print nrestarts,
                if (nrestarts % RESEED != 0):
                    deviation = 1 - 0.95**(nrestarts%RESEED)
                    N0_inv = N0_inv*np.random.uniform(1-deviation,1+deviation,np.shape(N0_inv))#+np.zeros(numdemes)
                    m0 = m0*np.random.uniform(1-deviation,1+deviation, np.shape(m0))#+np.zeros((numdemes*(numdemes-1))/2)
                else:
                    #print 'reseeding'
#                    N0 = np.random.uniform(1000,5000,numdemes)
                    N0_inv = np.random.uniform(5e-5, 1e-3, numdemes)
                    m0 = np.random.uniform(1e-8, 1e-3, (numdemes*(numdemes-1))/2)
                x0 = np.hstack((N0_inv,m0))
                xopt, fun, nit, nfev, status = bnm.fmin_bound(compute_Frob_norm_mig, x0, bounds=lims, args=(t[i], obs_rates[:,i], P0, pdslice), maxfun=100000, maxiter=10000, xtol=XTOL, ftol=FTOL, full_output=True, disp=False)
                if (fun < bestfval):
                    bestfval = fun
...
#                print nrestarts, fun, [1./x for x in xopt[0:numdemes]], xopt[numdemes:]
                nrestarts += 1
                #raise Exception("Cannot converge to zero using Bounded Nelder-Mead method. Check starting point.")
#            xopt = opt.fmin_powell(compute_Frob_norm_mig, x0, args=(t[i], obs_rates[:,i], P0), maxfun=1000000, maxiter=100000)
#            xopt = opt.fmin_l_bfgs_b(compute_Frob_norm_mig, x0, args=(t[i], obs_rates[:,i], P0), maxfun=1000000, approx_grad=True, bounds=lims, pgtol=1e-10, factr=10)[0]
#            print bestfval, i, bestxopt[0:numdemes], bestxopt[numdemes:] 
            ...
            if (len(popdict) < numdemes):
                ...
                for iii in xrange(numdemes):
                    modXopt[iii] = 1./modXopt[iii]
                xopts.append(modXopt)
                Ne_inv = bestxopt[0:numdemes]
                mtemp = bestxopt[numdemes:]                    
        # end of reestimation loop
...
def cond_to_psmc(condRates):
    ...
    assert(nr > nc) # nr must be num of timeslices
    marg_rates[0,:] = condRates[0,:]
    for i in xrange(1, nr):
        marg_rates[i,:] = condRates[i,:]*(1.-np.sum(marg_rates[0:i,:], 0))
    return marg_rates
...
def plot_truth(obs_rates, N1s, N2s, m, t, slicenum=0):
    ...
    P0 = np.matrix(np.eye(nr))
    N1s = [1./x for x in N1s]
    N2s = [1./x for x in N2s]
#    m = np.arange(0, 1e-3, 1e-6)
    cnt = 0
...
def find_pop_merges(Ninv, mtemp, t, P0, merge_threshold, useMigration):
    ...
    if (useMigration == False):
        ...
        P = expM(t*Q)[0:-1,-1]
        popdict = []
        for i in xrange(numdemes): popdict.append([])
...
#                print i,j, medianRates, meanRates
        ...
        popdict = []
        for kk in xrange(numdemes): popdict.append([])
        cnt = 0
        for ii in xrange(numdemes):
            for jj in xrange(ii+1, numdemes):
...
def comp_N_m_bfgs(obs_rates, t, merge_threshold, useMigration, initialize=True):
    """This function estimates the N and m parameters for the various time slices. The 
    time slices are given in a vector form 't'. t specifies the length of the time slice, not
    the time from present to end of time slice (not cumulative but atomic)
    Also, the obs_rates are given, for each time slice are given in columns of the obs_rates
    ...
    """
    #FIXED PARAMETERS FOR THIS FUNCTION
    FTOL = 1e1#moderate accuracy=1e7, high accuracy=1e1
    PTOL = 1e-20
    EPSILON = 1e-11
    RESTARTS = 20
    FLIMIT = 1e-12
    ...
    for i in xrange(numslices):
        ...
        if i > 0:
            x0 = bestxopt.copy()
            xopt, fun, dic = opt.fmin_l_bfgs_b(compute_Frob_norm_mig, x0, bounds=lims, args=(t[i], obs_rates[:,i], P0, make_merged_pd(pdlist)), approx_grad=True, maxfun=100000, factr=FTOL, epsilon=EPSILON, disp=0)
            bestxopt = xopt
            bestfval = fun
#            for lll in xrange((numdemes*(numdemes-1))/2):
#                x0 = bestxopt.copy()
...
#        update the P0 matrix - have to update this to mean different things
        ...
        while(reestimate):
            ...
            if initialize:
                ...
                x0 = np.hstack((N0_inv, m0))
            lims = [(1e-15, 1e-1)]*numdemes
            lims += [(1e-15,1e-1)]*((numdemes*(numdemes-1))/2)
            try:
                xopt, fun, dic = opt.fmin_l_bfgs_b(compute_Frob_norm_mig, x0, bounds=lims, args=(t[i], obs_rates[:,i], P0, make_merged_pd(pdlist)), approx_grad=True, maxfun=100000, factr=FTOL, epsilon=EPSILON, iprint=-1)
            ...
            if (len(popdict) < numdemes):
                ...
                modXopt = [1./x for x in bestxopt[0:numdemes]]
                cnt = 0
                for ii in xrange(numdemes):
                    for jj in xrange(ii+1, numdemes):
...
#            for lll in xrange((numdemes*(numdemes-1))/2):
#                x01 = x0[0:nr-1].copy()
#                x01[numdemes+lll] = 9.9e-3
#                try:
#                    xopt, fun, dic = opt.fmin_l_bfgs_b(compute_Frob_norm_mig, x01, bounds=lims, args=(t[i], obs_rates[:,i], P0, make_merged_pd(pdlist)), approx_grad=True, maxfun=100000, factr=FTOL, epsilon=EPSILON, disp=0)
#                    if (fun < bestfval):
#                        bestfval = fun
#                        bestxopt = xopt
#                except Exception as e:
...
#        update the P0 matrix - have to update this to mean different things
        ...
        while(reestimate):
            ...
            lims += [(1e-15,1e-1)]*((numdemes*(numdemes-1))/2)
            try:
                xopt = opt.fmin_cobyla(compute_Frob_norm_mig, x0, bounds=lims, args=(t[i], obs_rates[:,i], P0, make_merged_pd(pdlist)), approx_grad=True, maxfun=10000, pgtol=PTOL, xtol=PTOL, ftol=PTOL)
            ...
            nrestarts = 0
            try:
                xopt, nf, rc = opt.fmin_tnc(compute_Frob_norm_mig, x0, bounds=lims, args=(t[i], obs_rates[:,i], P0, make_merged_pd(pdlist)), approx_grad=True, maxfun=10000, pgtol=PTOL, xtol=PTOL, ftol=PTOL)
                fun = compute_Frob_norm_mig(xopt, t[i], obs_rates[:,i], P0, make_merged_pd(pdlist))
                if (fun < bestfval):
                    bestxopt = xopt
                    bestfval = fun
